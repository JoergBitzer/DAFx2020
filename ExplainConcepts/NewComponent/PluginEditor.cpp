...
FilarborAudioProcessorEditor::FilarborAudioProcessorEditor (FilarborAudioProcessor& p, AudioProcessorValueTreeState& vts)
    ...
    m_minidisp(paramLpCutoff.minValue,paramLpCutoff.maxValue)
{
    ...
    addAndMakeVisible(m_minidisp);
    m_minidisp.onValueChange = [this]() {double or = m_minidisp.getLPorder(); m_orderLpSlider.setValue(or , NotificationType::sendNotification);
    double freq = m_minidisp.getFreqLP(); m_cutoffLpSlider.setValue(freq, NotificationType::sendNotification); };
...
#define GUI_DISPLAY_WIDTH 300
#define GUI_DISPLAY_HEIGHT 100
...
void FilarborAudioProcessorEditor::resized()
{
    ...
    m_minidisp.setBounds(r.getWidth() / 2 - GUI_DISPLAY_WIDTH / 2, r.getHeight() / 2 - GUI_DISPLAY_HEIGHT / 2, GUI_DISPLAY_WIDTH, GUI_DISPLAY_HEIGHT);
