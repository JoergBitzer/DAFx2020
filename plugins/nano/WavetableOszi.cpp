...
WavetableOszi::WavetableOszi()
	...
	m_portamentorTime_Samples(-1),
	m_portamentoFactor(1.0),
	m_incStepRelativeFactor(1.0)
{
...
int WavetableOszi::getData(std::vector<double>& data)
{
	...
	for (auto kk = 0; kk < data.size(); kk++)
	{
		...
		//m_incStep *= m_incStepRelativeFactor;
		...
		m_curPos += (m_incStep*m_incStepRelativeFactor);
...
{
	m_freq = freq;
	if (m_portamentorTime_Samples <= 0)
	{
		computeIncStep(m_freq);
		m_portamentoCounter = -1;
	}
	...
	{
		m_portamentoCounter = m_portamentorTime_Samples;
		double targetInc = m_freq / m_f0Table;
		m_portamentoFactor = exp(log(targetInc / m_incStep) / m_portamentorTime_Samples);
	}
...
void WavetableOszi::computeIncStep(double freq)
{
	m_incStep = freq / m_f0Table;
}
...
void WavetableOszi::changeFrequencyRelative(double deltaFreq)
{

	//computeIncStep(m_freq+deltaFreq);
	//double curFreq = m_incStep * m_f0Table;
	//double newfreq = curFreq * pow(2.0, double(deltaFreq) / 12.0);
	m_incStepRelativeFactor = pow(2.0, double(deltaFreq) / 12.0);
	//m_incStep = (newfreq) / m_f0Table;

	//m_curWavetable = static_cast<int> (log((m_freq+deltaFreq) / m_f0Table) / log(pow(2.0, double(m_semitonespertable) / 12.0)));
	//if (m_curWavetable < 0)
	//	m_curWavetable = 0 ;

}

void WavetableOszi::setPortamentoTime(double newPortamento_ms)
{
	m_portamentorTime_Samples = static_cast<int> (newPortamento_ms * 0.001 * m_fs);
}
